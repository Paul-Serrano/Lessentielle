...
class Colissimo extends CarrierModule
{
    ...
    public function postProcessConfiguration()
    {
        ...
        } elseif (Tools::isSubmit('submitColissimoEnableDDP')) {
            $this->context->smarty->assign('active', 'fo');
            $this->postProcessEnableDDP();
     ...
     *
     */
    public function postProcessEnableDDP()
    {
        Configuration::updateValue('COLISSIMO_ENABLE_DDP', Tools::getValue('COLISSIMO_ENABLE_DDP'));
        Configuration::updateValue('COLISSIMO_DDP_COST', Tools::getValue('COLISSIMO_DDP_COST'));
        Configuration::updateValue('COLISSIMO_DDP_GB_COST', Tools::getValue('COLISSIMO_DDP_GB_COST'));
        $this->context->controller->confirmations = $this->l('Delivery duty paid configuration saved successfully.');
    }
    ...
    public function hookDisplayPaymentTop($params)
    {
        if (Tools::version_compare(_PS_VERSION_, 1.7, '<')){
            ...
            $cart = $params['cart'];
            $carrier = new Carrier((int) $cart->id_carrier);
            $service = ColissimoService::getServiceTypeByIdCarrier($carrier->id_reference);
            if (Configuration::get('PS_ORDER_PROCESS_TYPE')){
                if ($service == ColissimoService::TYPE_RELAIS){
                ...
                $colissimoDDP = 0;
                // Assign ddp cost
                if (Configuration::get('COLISSIMO_ENABLE_DDP') && in_array($isoCountryCustomer, ColissimoTools::$isoDDP) && $carrier->is_free == 0) {
                    $colissimoDDP = Configuration::get("COLISSIMO_DDP_COST");
                    if ($isoCountryCustomer == "GB") {
                        $colissimoDDP = Configuration::get("COLISSIMO_DDP_GB_COST");
                    }
