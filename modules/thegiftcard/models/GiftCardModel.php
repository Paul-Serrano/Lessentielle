...
class GiftCardModel extends ObjectModel
{
    ...
    {
        $ids = [];
        $gift_cards = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
        SELECT cr.code
        FROM `' . _DB_PREFIX_ . 'giftcard` gc
    ...
    public static function getStatistics($module)
    {
        ...
        foreach ($result as &$row) {
            ...
            $row['cart_rule_url'] = $module->getAdminLink('AdminCartRules', ['id_cart_rule' => $row['id_cart_rule'], 'updatecart_rule' => 1]);
            $row['order_url'] = $module->getAdminLink('AdminOrders', ['id_order' => $row['id_order'], 'vieworder' => 1]);
            $row['consumption'] = [];
    ...
    public static function getCurrenciesNotIndexed()
    {
        ...
        foreach ($currencies as $currency) {
            ...
            if (Validate::isLoadedObject($giftcard) && ($giftcard->isAssociatedToShop() || null === Shop::getContextShopID(true))) {
                continue;
            }
    ...
    {
        if (is_callable(['Attribute', 'isAttribute'])) {
