...
class ThegiftcardPageModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        if (Validate::isLoadedObject($this->product) && $this->product->isAssociatedToShop()) {
            ...
            if (isset($this->context->cookie, $this->category->id_category)
    ...
    protected function assignCategory()
    {
        ...
        if (false !== $this->category && Validate::isLoadedObject($this->category) && $this->category->inShop() && $this->category->isAssociatedToShop()) {
    ...
    public function displayAjaxGetCombination()
    {
        if (!Configuration::get('GIFTCARD_ACTIVE_' . (int) $this->product->id) || !$this->product->active) {
            die(Tools::jsonEncode(['error' => $this->module->l('The gift card is not active for the moment.', 'page')]));
        }
        ...
        if (GiftCardModel::PRINT_AT_HOME != $sending_method && GiftCardModel::SEND_TO_FRIEND != $sending_method) {
            die(Tools::jsonEncode(['error' => $this->module->l('Please select a sending method', 'page')]));
        }
        ...
        if (!isset($attributes)
            ...
            || !is_array($attributes)
            || !count($attributes)) {
            die(Tools::jsonEncode(['error' => $this->module->l('Please select a template and an amount', 'page')]));
        }

        if (!($id_template = $this->filterAttribute($attributes, Configuration::get('GIFTCARD_ATTRGROUP_TEMPLATE'), $this->getImagesList()))) {
            die(Tools::jsonEncode(['error' => $this->module->l('Please select a template', 'page')]));
        }

        if (!($id_amount = $this->filterAttribute($attributes, Configuration::get('GIFTCARD_ATTRGROUP_AMOUNT'), $this->getAmountsList()))) {
            die(Tools::jsonEncode(['error' => $this->module->l('Please select an amount', 'page')]));
        }

        if (!($id_combination = $this->module->generateCombination($this->product->id, [$id_template, $id_amount]))) {
            die(Tools::jsonEncode(['error' => $this->module->l('Error while creating the gift card with selected template and amount', 'page')]));
        }

        $combination = new Combination($id_combination);
        if (!Validate::isLoadedObject($combination)) {
            die(Tools::jsonEncode(['error' => $this->module->l('Unable to load the combination object', 'page')]));
        }
        ...
        if (GiftCardModel::SEND_TO_FRIEND == $sending_method) {
            ...
            foreach (Tools::getValue('customizationData') as $field_name => $value) {
                if (in_array($field_name, $authorized_text_fields) && '' != $value) {
                    if ($indexes[$field_name] == Configuration::get('GIFTCARD_CUST_DATE_' . (int) $this->product->id) && (!($value = date('Y-m-d', strtotime($value))) || !Validate::isDate($value) || date('Y-m-d') > $value)) {
                        die(Tools::jsonEncode(['error' => $this->module->l('Please select a valid date', 'page')]));
                    } elseif ($indexes[$field_name] == Configuration::get('GIFTCARD_CUST_EMAIL_' . (int) $this->product->id) && !Validate::isEmail($value)) {
                        die(Tools::jsonEncode(['error' => $this->module->l('Please fill a valid email', 'page')]));
                    } elseif (!Validate::isMessage($value)) {
                        die(Tools::jsonEncode(['error' => $this->module->l('An error occurred while attempting to save this data.', 'page')]));
                    } else {
                    ...
                    die(Tools::jsonEncode(['error' => $this->module->l('Please fill all the fields.', 'page')]));
                }
            }
            ...
            if (empty($customization_datas)) {
                $combination->delete();
                die(Tools::jsonEncode(['error' => $this->module->l('An error occurred while attempting to get the customized data.', 'page')]));
            }
    ...
    public function displayAjaxGeneratePdf()
    {
        if (!GiftCardModel::isPDFFeatureActive()) {
            die(Tools::jsonEncode(['error' => $this->module->l('The preview feature is not active for the moment.', 'page')]));
        }
        ...
        if (!($id_image = $this->filterAttribute($attributes, Configuration::get('GIFTCARD_ATTRGROUP_TEMPLATE'), $this->getImagesList(), true))) {
            die(Tools::jsonEncode(['error' => $this->module->l('Please select a template', 'page')]));
        }

        if (!($amount = $this->filterAttribute($attributes, Configuration::get('GIFTCARD_ATTRGROUP_AMOUNT'), $this->getAmountsList(), true))) {
            die(Tools::jsonEncode(['error' => $this->module->l('Please select an amount', 'page')]));
        }
